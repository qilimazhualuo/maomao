  ...
  () => {
    ...
    const warehouseStore = computed(() => {
      ...
      return warehouses.value.reduce(
        ...
        { food: 1000, wood: 1000, stone: 200, gold: 200 }
      ) // 基础容量
    })
      ...
      Object.keys(structures).forEach((key) => {
        ...
        consume &&
          Object.keys(consume).forEach((resource) => {
            resourceState[resource] -= consume[resource] * structures[key].length
          })
        // 建筑周期产出
        const product = config[key].product
        product &&
          Object.keys(product).forEach((resource) => {
            resourceState[resource] += product[resource] * structures[key].length
          })
      })
      ...
      switch (type) {
        ...
        case 'warehouse':
          ...
          Object.keys(config.warehouse.value).forEach((key) => {
            resourceState[key] -= config.warehouse.value[key]
          })
      }
    }
